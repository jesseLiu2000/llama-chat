...
def get_helm_data_list(file_path, prepend_text, k, tokenizer, context_window, num_examples=5, batch_size=1, max_gen_len=100, num_instances = 0):
    ...
    for idx in idx_lst:
        instructions = file_content[idx]["instructions"] + "The dataset name is " + file_path.split("/")[-1][:-5]
        # instructions = ""
        text = file_content[idx]["input_text"]
        few_shot = file_content[idx]["few_shots"]
        post_fs_text = "I'll remember this example as I complete similar prompts."
        ...
        input_list.append({"input": truncated_input, "instance_id": idx})
...
def get_msmacro_data_list(file_path, prepend_text, k, tokenizer, context_window, num_examples=5, batch_size=1, max_gen_len=100, num_instances = 0):
    """Given a path to a csv storing inputs taken in HELM format, will create a prompt for each instance to pass to the model. 
